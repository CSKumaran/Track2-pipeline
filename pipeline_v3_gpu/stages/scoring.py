"""Stage 7: Asymmetric Temporal Contiguity Scoring [v2.2].
...
Asymmetric rule (applied BEFORE formula):
  if delta_t ≤ 0: S = 100 (no penalty — visual was already on screen)
  if delta_t > 0: S = formula(delta_t)

Zone classification:
  delta_t ≤ 0.0  → Optimal (visual precedes narration)
...
def gaussian_score(delta_t: float, tau: float = 2.5,
                   match_case: str = None) -> float:
    """Gaussian temporal score. Asymmetric with visual-presence verification.

    delta_t ≤ 0 → Score = 100 ONLY if match_case is A or D (visual verified
    on screen at t_narr). For other cases with negative delta_t (shouldn't
    normally occur), apply the Gaussian penalty using |delta_t| as a safety net.
    """
    if delta_t is None or pd.isna(delta_t):
        return None
    if delta_t <= 0:
        # Visual is BEFORE narration — but was it still on screen?
        # Cases A and D verified visual presence at t_narr → safe, no penalty
        # Case B/C/G should never produce negative delta_t, but guard anyway
        if match_case in ("A", "D", None):
            return 100.0
        else:
            # Unexpected negative delta_t from non-verified case — use |delta_t|
            d = abs(delta_t)
            return 100.0 * np.exp(-0.5 * (d / tau) ** 2)
    return 100.0 * np.exp(-0.5 * (delta_t / tau) ** 2)


def piecewise_score(delta_t: float, match_case: str = None) -> float:
    ...
    delta_t ≤ 0 → 100 ONLY for Cases A/D (visual verified on screen).
    """
    if delta_t is None or pd.isna(delta_t):
        return None
    if delta_t <= 0:
...
def zone_classification(delta_t: float, match_case: str = None) -> str:
    """Classify temporal gap into cognitive zones. Asymmetric with verification.

    delta_t ≤ 0 from Case A/D → Optimal (visual verified on screen).
    delta_t ≤ 0 from other cases → uses |delta_t| for zone.
    """
    if delta_t is None or pd.isna(delta_t):
        return "No Match"
    if delta_t <= 0:
        if match_case in ("A", "D", None):
            return "Optimal"  # Visual verified on screen at t_narr
        else:
            d = abs(delta_t)
            if d <= 1.0:
                return "Optimal"
...
def run_stage7(output_dir: str, cfg: Config, diag=None) -> dict:
    ...
    if kw_df.empty:
        ...
        save_json({"pipeline_version": "2.2", "error": "no_data"},
...
def _score_keywords(kw_df: pd.DataFrame, tau: float, scoring_mode: str,
                    floor_factor: float) -> pd.DataFrame:
    """Add S_gaussian, S_piecewise, S_temporal, zone to each keyword."""
    kw_df = kw_df.copy()

    # Gaussian score (always computed) — passes match_case for asymmetric verification
    kw_df["S_gaussian"] = kw_df.apply(
        lambda r: gaussian_score(r["delta_t"], tau,
                                 match_case=r.get("match_case")), axis=1
    )
    ...
    # Zone classification — passes match_case for asymmetric verification
