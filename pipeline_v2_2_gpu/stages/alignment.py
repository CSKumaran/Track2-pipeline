...
def _case_a_visual_present(kw_text: str, t_narr: float,
    ...
    # Sub-case D check: was keyword there from scene start, or mid-scene reveal?
    # Check FIRST HALF of scene frames (first 30% of scene duration) for keyword
    first_segment_end = scene_t_start + (scene_t_end - scene_t_start) * 0.3
    early_frames_have_kw = False

    for frame_t, word_set in sorted(ocr_word_sets.items()):
        if scene_t_start <= frame_t <= first_segment_end and word_set:
            # Check this early frame
            if fuzzy_match_multiword(kw_text, word_set, threshold):
                early_frames_have_kw = True
                break
            elif is_word_match:
                ocr_norm = {normalize_word(w) for w in word_set if len(w) >= 3}
                kw_words_check = [normalize_word(w) for w in kw_text.split()
                                  if len(normalize_word(w)) >= 4]
                for kw_w in kw_words_check:
                    if fuzzy_match_word(kw_w, ocr_norm, threshold):
                        early_frames_have_kw = True
                        break
                if early_frames_have_kw:
                    break

    if early_frames_have_kw:
        # Standard Case A: keyword was there from scene start
        t_vis = scene_t_start
        match_case = "A"
        sub_scene_onset = None
    else:
        # Case D: Progressive reveal — keyword appeared mid-scene
        # Find earliest OCR frame within scene that has the keyword
        earliest_t = _find_earliest_ocr_in_scene(
            kw_text, scene_t_start, t_narr, ocr_word_sets, threshold
        )
        t_vis = earliest_t if earliest_t is not None else t_narr
        match_case = "D"
        sub_scene_onset = t_vis

    delta_t = t_vis - t_narr  # ≤ 0 for Case A, ≤ 0 for Case D
...
def _find_earliest_ocr_in_scene(kw_text: str, scene_start: float,
                                t_narr: float, ocr_word_sets: dict,
                                threshold: float) -> float:
    """Find earliest frame in [scene_start, t_narr] with keyword (full or word-level)."""
    candidates = []
    kw_words = [normalize_word(w) for w in kw_text.split()
                if len(normalize_word(w)) >= 4]

    for frame_t, word_set in ocr_word_sets.items():
        if scene_start <= frame_t <= t_narr and word_set:
            # Full match
            if fuzzy_match_multiword(kw_text, word_set, threshold):
