"""Dashboard generator for Pipeline v4.1 — Temporal Contiguity Analysis.
...
def generate_dashboard(output_dir: str, cfg=None) -> str:
    ...
    html_parts = [
        ...
        _section_pedagogical_importance(data),
...
<p style="color: #7f8c8d; margin-bottom: 20px;">Pipeline v4.1 | Transcript-First Architecture | Asymmetric Scoring</p>
...
# ═══════════════════════════════════════════════════════════════════════════════
# Section 1b: Pedagogical Importance
# ═══════════════════════════════════════════════════════════════════════════════

IMP_COLORS = {
    5: "#1e8449",  # deep green — critical
    4: "#27ae60",  # green — above average
    3: "#f39c12",  # yellow — average
...
IMP_WEIGHT_MAP = {1: 0.8, 2: 0.9, 3: 1.0, 4: 1.1, 5: 1.2}


def _section_pedagogical_importance(data: dict) -> str:
    """Section showing pedagogical importance ratings per segment."""
    importance = data.get("importance", pd.DataFrame())
    segments = data.get("segments", pd.DataFrame())

    if importance.empty or segments.empty:
        return '<h2 id="importance">Pedagogical Importance</h2><div class="card"><p>No importance data available.</p></div>'

    # Merge segments with importance
    rows_html = []
    ...
    for _, seg in segments.iterrows():
        ...
        imp_row = importance[importance["segment_id"] == sid]
        if not imp_row.empty:
            imp_row = imp_row.iloc[0]
            imp_val = int(imp_row.get("importance", 3))
            reason = str(imp_row.get("reason", "")) if pd.notna(imp_row.get("reason")) else ""
            backend = str(imp_row.get("backend", "unknown")) if pd.notna(imp_row.get("backend")) else "unknown"
            is_reliable = imp_row.get("is_reliable", True)
        else:
            imp_val = 3
            reason = ""
            backend = "default"
        ...
        dist[imp_val] = dist.get(imp_val, 0) + 1
        weight = IMP_WEIGHT_MAP.get(imp_val, 1.0)
        color = IMP_COLORS.get(imp_val, "#95a5a6")
        label = IMP_LABELS.get(imp_val, "Unknown")
        reliability = "" if is_reliable else ' <span style="color:#e74c3c;font-size:0.75em;">⚠ unreliable</span>'
        ...
        rows_html.append(f"""
        <tr>
            <td style="text-align:center;font-weight:600;">{sid}</td>
            <td style="font-size:0.8em;color:#7f8c8d;">{_fmt_time(t0)} – {_fmt_time(t1)}</td>
            <td style="font-size:0.85em;">{text}{"..." if len(str(seg.get("text", ""))) > 80 else ""}</td>
            <td style="text-align:center;">
                <span style="display:inline-block;padding:3px 10px;border-radius:12px;
                             background:{color};color:white;font-weight:700;font-size:0.85em;">
                    {imp_val}</span>
                <div style="font-size:0.7em;color:{color};margin-top:2px;">{label}</div>
            </td>
            <td style="text-align:center;font-size:0.85em;">{weight:.1f}x</td>
            <td style="font-size:0.75em;color:#7f8c8d;">{backend}{reliability}</td>
            <td style="font-size:0.8em;">{reason_short}</td>
        </tr>""")
    ...
    for imp in [5, 4, 3, 2, 1]:
        count = dist.get(imp, 0)
        pct = count / total * 100 if total > 0 else 0
        if count > 0:
            color = IMP_COLORS.get(imp, "#95a5a6")
            dist_bar.append(
                f'<div style="flex:{pct};background:{color};color:white;text-align:center;'
                f'padding:4px 2px;font-size:0.75em;font-weight:600;" '
                f'title="{IMP_LABELS.get(imp, "")}: {count} segments ({pct:.0f}%)">'
                f'{imp} ({count})</div>'
            )

    # Backend summary
    backends = importance["backend"].value_counts().to_dict() if "backend" in importance.columns else {}
    backend_text = ", ".join(f"{b}: {c}" for b, c in backends.items())

    return f"""
<h2 id="importance">Pedagogical Importance</h2>

<div class="card">
    <p style="margin-bottom:10px;"><strong>Importance Distribution</strong>
        <span style="font-size:0.8em;color:#7f8c8d;margin-left:10px;">
        Rated by: {backend_text if backend_text else 'unknown'}</span></p>
    <div style="display:flex;border-radius:6px;overflow:hidden;height:28px;margin-bottom:15px;">
        {"".join(dist_bar)}
    </div>
    <p style="font-size:0.8em;color:#7f8c8d;">
        5 = Critical (skipping makes later content incomprehensible) &nbsp;|&nbsp;
        4 = Above Average (key concept introductions) &nbsp;|&nbsp;
        3 = Average (supporting material) &nbsp;|&nbsp;
        2 = Below Average (repetition, tangential) &nbsp;|&nbsp;
        1 = Filler (greetings, transitions)</p>
</div>
    ...
    <table style="width:100%;border-collapse:collapse;font-size:0.9em;">
        <thead>
            <tr style="border-bottom:2px solid #bdc3c7;text-align:left;">
                <th style="padding:8px;width:40px;">Seg</th>
                <th style="padding:8px;width:90px;">Time</th>
                <th style="padding:8px;">Transcript</th>
                <th style="padding:8px;width:80px;text-align:center;">Rating</th>
                <th style="padding:8px;width:50px;text-align:center;">Weight</th>
                <th style="padding:8px;width:80px;">Backend</th>
...
def _section_segment_details(data: dict, output_dir: str) -> str:
    ...
    for _, seg in segments.iterrows():
        ...
        imp_reason = str(imp_row["reason"]) if imp_row is not None and pd.notna(imp_row.get("reason")) else ""
        imp_backend = str(imp_row["backend"]) if imp_row is not None and pd.notna(imp_row.get("backend")) else ""
        imp_color = IMP_COLORS.get(imp_val, "#95a5a6")
        imp_label = IMP_LABELS.get(imp_val, "")
        imp_weight = IMP_WEIGHT_MAP.get(imp_val, 1.0)
        ...
        <div class="{card_cls}">
            ...
            <div style="display:flex; gap:15px; font-size:0.8em; color:#7f8c8d; margin-bottom:8px; align-items:center;">
                <span>Keywords: {n_matched}/{n_kw} matched ({match_rate:.0%})</span>
                <span>Importance:
                    <span style="display:inline-block;padding:1px 8px;border-radius:10px;
                                 background:{imp_color};color:white;font-weight:700;font-size:0.9em;">
                        {imp_val}/5</span>
                    <span style="color:{imp_color};font-size:0.85em;">{imp_label}</span>
                    (weight: {imp_weight:.1f}x)
                </span>
            </div>
            {"" if not imp_reason else f'<div style="font-size:0.75em;color:#555;margin-bottom:8px;padding:4px 8px;background:#f0f4f8;border-radius:4px;border-left:3px solid {imp_color};"><em>Importance reason ({imp_backend}):</em> {imp_reason[:150]}{"..." if len(imp_reason) > 150 else ""}</div>'}
...
def _section_top5_fixes(data: dict, output_dir: str) -> str:
    ...
    for i, fix in enumerate(fixes):
        ...
        dt_abs = abs(dt) if dt else 0

        # Severity label
        if dt and dt > 5:
            severity = "Critical"
            severity_cls = "high-priority"
        elif dt and dt > 3:
            severity = "High"
            severity_cls = "high-priority"
        elif dt and dt > 1:
            severity = "Moderate"
            severity_cls = "medium-priority"
        else:
            severity = "Low"
            severity_cls = "medium-priority"
        ...
        if s_temp is not None:
            if s_temp >= 80:
                ...
                align_label = "Very Poor"
        else:
            align_label = "N/A"

        # Importance context for this segment
        imp_df = data.get("importance", pd.DataFrame())
        fix_imp_val = 3
        fix_imp_reason = ""
        fix_imp_weight = 1.0
        if not imp_df.empty and seg_id in imp_df["segment_id"].values:
            fix_imp_row = imp_df[imp_df["segment_id"] == seg_id].iloc[0]
            fix_imp_val = int(fix_imp_row.get("importance", 3))
            fix_imp_reason = str(fix_imp_row.get("reason", "")) if pd.notna(fix_imp_row.get("reason")) else ""
            fix_imp_weight = IMP_WEIGHT_MAP.get(fix_imp_val, 1.0)
        fix_imp_color = IMP_COLORS.get(fix_imp_val, "#95a5a6")

        # Actionable recommendation
        if dt and dt > 0:
            rec_text = (
                f'Students hear "<strong>{kw_text}</strong>" but the visual '
                f'doesn\'t appear for another <strong>{dt_abs:.1f} seconds</strong>. '
                f'Move the slide transition approximately {dt_abs:.0f}s earlier so the visual '
                f'is on screen before or when this concept is narrated.'
            )
        else:
            rec_text = (
                f'The visual for "<strong>{kw_text}</strong>" appears '
                f'{dt_abs:.1f}s before narration. This is acceptable but flagged '
                f'due to low confidence or importance weighting.'
            )

        cards.append(f"""
        <div class="card">
            <h3>#{i+1}: "{kw_text}" <span class="zone zone-{zone_cls}">{zone}</span></h3>
            <p style="font-size:0.85em;color:#7f8c8d;margin:5px 0;">
                <strong>Severity: {severity}</strong> &mdash;
                visual appears {dt_display} {'after' if dt and dt > 0 else 'before'} narration &nbsp;|&nbsp;
                Alignment: {align_label} ({f'{s_temp:.0f}' if s_temp else 'N/A'}/100) &nbsp;|&nbsp;
                Importance: <span style="padding:1px 6px;border-radius:8px;background:{fix_imp_color};
                    color:white;font-weight:700;font-size:0.9em;">{fix_imp_val}/5</span>
                (weight: {fix_imp_weight:.1f}x)
            </p>
            {"" if not fix_imp_reason else f'<p style="font-size:0.8em;color:#555;margin:4px 0;"><em>Why important:</em> {fix_imp_reason[:120]}</p>'}
            <p style="font-size:0.85em;margin:8px 0;background:#f8f9fa;padding:8px;border-radius:4px;">
                <em>Narration:</em> "{seg_text}..."</p>
            <div class="fix-card" style="margin-top:10px;">
                <div class="fix-frame">
                    <p style="font-size:0.8em;font-weight:600;">
                        When concept is spoken ({_fmt_time(t_narr_val)})</p>
                    <p style="font-size:0.75em;color:#7f8c8d;">
                        What the student sees at this moment</p>
                    {frame_narr_html}
                </div>
                <div class="fix-frame">
                    <p style="font-size:0.8em;font-weight:600;">
                        When visual appears ({_fmt_time(t_vis_val)})</p>
                    <p style="font-size:0.75em;color:#7f8c8d;">
                        The matching visual &mdash; {dt_display} {'later' if dt and dt > 0 else 'earlier'}</p>
                    {frame_vis_html}
                </div>
            </div>
            <div class="rec {severity_cls}" style="margin-top:10px;">
...
<p style="color:#7f8c8d;margin-bottom:15px;">
    Ranked by: how poor the alignment is &times; pedagogical importance &times; match confidence.
    These are the highest-impact timing issues to fix.</p>
{"".join(cards)}
"""
...
<div style="text-align:center; margin-top:40px; padding:20px; color:#95a5a6; font-size:0.8em;">
    <p>Generated by Pipeline v4.1 — Temporal Contiguity Analysis</p>
