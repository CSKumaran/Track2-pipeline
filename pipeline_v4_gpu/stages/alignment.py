"""Stage 4: Transcript-First Alignment [v4.0].
...
Search Priority Order (per keyword):
  ...
  4. Case E: Gemini concept match [v4.0] → VLM-identified concepts → alpha 0.6-0.9
  5. Case G: SigLIP similarity → visual match without text → reduced alpha
  6. Case F: No visual correlate → excluded from scoring

Asymmetric scoring (visual persistence verified):
  - delta_t = t_vis - t_narr
  - delta_t ≤ 0 AND visual still on screen → NO penalty (score 100)
  - delta_t ≤ 0 AND visual no longer on screen → penalized (Case C)
  - delta_t > 0 → visual appears AFTER narration → penalized

  Visual persistence is verified per case:
  - Case A: keyword in current scene's OCR union → scene active → on screen ✓
  - Case D: progressive reveal in current scene → scene active → on screen ✓
  - Case C: scene ended before t_narr → visual gone → NOT on screen
  - Case E/G: check if matched scene.t_start ≤ t_narr ≤ scene.t_end

Outputs:
    keyword_alignment.csv — per-keyword: t_narr, t_vis, delta_t, match_case, alpha,
                             visual_on_screen (persistence verification flag)
...
def run_stage4(output_dir: str, cfg: Config, diag=None) -> dict:
    ...
    # [v4.0] Transcript-informed OCR correction
    transcript_words_path = os.path.join(output_dir, "transcript_words.csv")
    if os.path.exists(transcript_words_path):
        from ..utils.ocr_utils import (
            build_transcript_vocabulary, correct_ocr_with_transcript,
            build_abbreviation_map, expand_abbreviations_in_ocr,
        )
        transcript_vocab = build_transcript_vocabulary(transcript_words_path)
        abbrev_map = build_abbreviation_map(transcript_words_path)
        if transcript_vocab:
            n_corrected = 0
            for frame_t in ocr_word_sets:
                original = ocr_word_sets[frame_t]
                if original:
                    corrected = correct_ocr_with_transcript(original, transcript_vocab)
                    corrected = expand_abbreviations_in_ocr(corrected, abbrev_map)
                    if len(corrected) > len(original):
                        n_corrected += 1
                    ocr_word_sets[frame_t] = corrected
            logger.info("[v4.0] Transcript-informed OCR: corrected %d/%d frames",
                        n_corrected, len(ocr_word_sets))
    ...
    for _, kw_row in keywords_df.iterrows():
        ...
        # === Case E: Gemini concept match [v4.0] ===
        if getattr(cfg, 'TRACK_E_ENABLED', False) and "gemini_concepts" in scenes_df.columns:
            result = _case_e_gemini_concept(
            ...
            if result is not None:
                ...
                alignment_results.append(result)
                diag_details.append({"kw_id": kw_id, "case": "E", "success": True})
                continue
...
def _case_a_visual_present(kw_text: str, t_narr: float,
    ...
    Visual persistence verification:
    - The current scene CONTAINS t_narr (scene.t_start ≤ t_narr ≤ scene.t_end)
    - Therefore the visual IS on screen when the student hears the narration
    - This is true simultaneous presentation → score 100 is justified

    If found:
    - t_vis = earliest OCR frame with keyword (for temporal precision)
    - delta_t = t_vis - t_narr ≤ 0 → Score 100 (visual verified on screen)
    - visual_on_screen = True (always, for Cases A and D)
    ...
    return {
        ...
        "visual_on_screen": True,  # [v4.0] Scene contains t_narr → verified
    }
...
def _case_b_forward_search(kw_text: str, t_narr: float,
    ...
    return {
        ...
        "sub_scene_onset": None,
        "confidence": confidence,
        "visual_on_screen": False,  # [v4.0] Visual appears after t_narr
    }
...
def _case_c_backward_search(kw_text: str, t_narr: float,
    ...
    return {
        ...
        "visual_on_screen": False,  # [v4.0] Scene ended before t_narr → expired
    }


# =====================================================================
# [v4.0] Case E: Gemini Concept Match
# =====================================================================

def _case_e_gemini_concept(kw_text: str, t_narr: float,
                           search_window: float,
                           scenes_df: pd.DataFrame,
                           cfg: Config) -> dict:
    """Case E: Match keyword against Gemini-extracted concepts from keyframes.

    [v4.0] Gemini frame understanding identifies concepts (diagram labels,
    formula components, visual elements) that OCR misses. This track catches
    keywords where the concept IS visually present but not as extractable text.

    Priority: after A/B/C (OCR), before G (SigLIP).
    """
    ...
    fuzzy_threshold = getattr(cfg, 'TRACK_E_FUZZY_THRESHOLD', 0.75) * 100  # rapidfuzz uses 0-100

    best_match = None
    best_score = 0

    for _, scene in nearby.iterrows():
        concepts_raw = str(scene.get("gemini_concepts", ""))
        if not concepts_raw or concepts_raw == "nan":
            continue
        ...
        for concept in concepts:
            ...
            if kw_lower in concept or concept in kw_lower:
                score = 100
            else:
                # Fuzzy match (token_set_ratio handles word reordering)
                score = fuzz.token_set_ratio(kw_lower, concept)

            if score >= fuzzy_threshold and score > best_score:
                best_score = score
                t_vis = float(scene["t_start"])
                delta_t = t_vis - t_narr
                sc_start = float(scene["t_start"])
                sc_end = float(scene["t_end"])
                # [v4.0] Visual persistence: is this scene still active at t_narr?
                scene_contains_tnarr = (sc_start <= t_narr <= sc_end)
                best_match = {
                    "t_narr": t_narr,
                    "t_vis": t_vis,
                    "delta_t": delta_t,
                    "match_case": "E",
                    "match_method": f"gemini_concept_{('exact' if score == 100 else 'fuzzy')}",
                    "alpha": min(0.9, 0.6 + 0.3 * (score / 100)),  # 0.6-0.9 based on match quality
                    "scene_id": int(scene.get("scene_id", -1)),
                    "scene_t_start": sc_start,
                    "scene_t_end": sc_end,
                    "sub_scene_onset": None,
                    "confidence": "MEDIUM" if score < 90 else "HIGH",
                    "gemini_concept_matched": concept,
                    "gemini_match_score": round(score / 100, 3),
                    "visual_on_screen": scene_contains_tnarr,  # [v4.0] persistence verified
                }
...
def _case_g_siglip_match(kw_text: str, t_narr: float,
    ...
    # [v4.0] Visual persistence: is the matched scene active at t_narr?
    scene_contains_tnarr = (float(scene["t_start"]) <= t_narr <= float(scene["t_end"]))
...
def _no_match_result(kw_id, kw_text, seg_id, t_narr, groundability,
    ...
    return {
        ...
        "visual_on_screen": False,  # [v4.0] No match → no visual
    }
...
def _compute_segment_aggregates(kw_align_df: pd.DataFrame,
    ...
    for seg_id in segments_df["segment_id"].unique():
        ...
        rows.append({
            ...
            "n_case_E": case_counts.get("E", 0),  # [v4.0] Gemini concept match
