"""Stage 2: Scene Detection (Multi-Signal Fusion) [v4.0].
...
def run_stage2(video_path: str, output_dir: str, cfg: Config, diag=None) -> dict:
    ...
    # [v4.0] Gemini frame understanding — extract concepts from keyframes
    if getattr(cfg, 'GEMINI_FRAME_ENABLED', False) and cfg.GEMINI_API_KEY:
        _add_gemini_frame_concepts(scenes, cfg)
...
GEMINI_FRAME_PROMPT = (
    "You are analyzing a frame from an instructional video. "
    "List ALL concepts, terms, labels, variables, and ideas visible in this frame. "
    "Include: text on slides, diagram labels, graph axis labels, formula components, "
    "variable names, table headers, code identifiers, and visual concepts "
    "(e.g. 'flowchart', 'scatter plot', 'neural network diagram') that a student would identify.\n\n"
    "Return ONLY a JSON array of strings, no other text. Example:\n"
    '[\"gradient descent\", \"learning rate alpha\", \"cost function J\", \"scatter plot\"]'
)


def _add_gemini_frame_concepts(scenes: list, cfg: Config):
    """Query Gemini for each scene keyframe to extract visible concepts.

    Adds 'gemini_concepts' column to each scene dict — a pipe-separated
    list of concept strings identified by the VLM.
    """
    import json as _json
    from ..utils.gemini_utils import query_gemini_vision

    prompt = getattr(cfg, 'GEMINI_FRAME_PROMPT', GEMINI_FRAME_PROMPT)
    n_success = 0
    n_total = len(scenes)
    ...
    for scene in scenes:
        ...
        if not kf_path or not os.path.exists(kf_path):
            scene["gemini_concepts"] = ""
            scene["n_gemini_concepts"] = 0
            continue

        try:
            response = query_gemini_vision(prompt, kf_path, cfg)
            concepts = _parse_concept_list(response)
            scene["gemini_concepts"] = "|".join(concepts)
            scene["n_gemini_concepts"] = len(concepts)
            if concepts:
                n_success += 1
        except Exception as e:
            logger.warning("Gemini frame query failed for scene %d: %s",
                           scene["scene_id"], e)
            scene["gemini_concepts"] = ""
            scene["n_gemini_concepts"] = 0

    logger.info("[v4.0] Gemini frame understanding: %d/%d scenes with concepts",
                n_success, n_total)


def _parse_concept_list(response_text: str) -> list:
    """Parse Gemini response into a list of concept strings."""
    import json as _json
    ...
    # Fallback: split by newlines/commas, strip bullets and quotes
    concepts = []
