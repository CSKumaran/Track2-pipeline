"""Pipeline v4.0 -- CLI entry point & stage orchestration."""
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Pipeline v4.0 -- Temporal Contiguity Analysis"
    )
    # Input/Output
    ...
    # [v4.0] Local VLM for Tier 2 importance
    p.add_argument("--local-vlm-backend", default="auto",
                   choices=["vllm", "ollama", "auto"],
                   help="Local VLM serving backend (vllm recommended for HPC)")
    p.add_argument("--local-vlm-endpoint", default="http://localhost:8000/v1",
                   help="OpenAI-compatible endpoint for vLLM/SGLang")
    p.add_argument("--local-vlm-model", default="Qwen/Qwen2.5-7B-Instruct",
                   help="Model ID for vLLM (e.g., Qwen/Qwen2.5-7B-Instruct)")
    ...
    # Dwell decay [v4.1]
    p.add_argument("--dwell-threshold", type=float, default=15.0,
                   help="Seconds before dwell decay starts for visual_on_screen")
    p.add_argument("--dwell-tau", type=float, default=30.0,
                   help="Gaussian width for dwell time decay")
    ...
    # [v4.0] New features
    p.add_argument("--no-gemini-frame", action="store_true",
                   help="Disable Gemini frame understanding (concept extraction)")
    p.add_argument("--no-track-e", action="store_true",
                   help="Disable Track E (Gemini concept matching in alignment)")
    p.add_argument("--first-mention-window", type=float, default=60.0,
                   help="Lookback window (seconds) for first-mention detection")

    # [v4.0] Sensitivity analysis
    p.add_argument("--sensitivity-analysis", action="store_true",
                   help="Run sensitivity analysis on heuristic weights after pipeline")
...
def build_config(args) -> Config:
    ...
    if args.gemini_model:
        cfg.GEMINI_MODEL = args.gemini_model  # CLI override; else use config.py default
    ...
    cfg.DWELL_THRESHOLD = args.dwell_threshold
    cfg.DWELL_TAU = args.dwell_tau
    cfg.OUTPUT_ROOT = args.output_root
    cfg.DIAGNOSTICS_ENABLED = not args.no_diagnostics
    cfg.MONOTONIC_CHECK_ENABLED = not args.no_monotonic_check
    # [v4.0]
    cfg.GEMINI_FRAME_ENABLED = not args.no_gemini_frame
    cfg.TRACK_E_ENABLED = not args.no_track_e
    cfg.FIRST_MENTION_WINDOW_S = args.first_mention_window
    cfg.LOCAL_VLM_BACKEND = args.local_vlm_backend
    cfg.LOCAL_VLM_ENDPOINT = args.local_vlm_endpoint
    cfg.LOCAL_VLM_MODEL_ID = args.local_vlm_model
    return cfg
...
def run_pipeline(video_path: str, cfg: Config, stage_only: int = None):
    ...
    # GPU cleanup before Stage 4 (SigLIP needs headroom after KeyBERT/embedding models)
    if should_run(4) or stage_only is None:
        import gc
        gc.collect()
        try:
            import torch
            if torch.cuda.is_available():
                ...
                logger.info("GPU cleanup before Stage 4: %d MB free", free_mb)
        except ImportError:
            pass
...
def main():
    ...
    logger.info("Pipeline v4.0 starting")
    ...
    logger.info("TEMPORAL_SIGMA=%.1f, USE_KEYFRAME_AS_TVIS=%s, SCORING_MODE=%s, DIAGNOSTICS=%s",
                cfg.TEMPORAL_SIGMA, cfg.USE_KEYFRAME_AS_TVIS, cfg.SCORING_MODE, cfg.DIAGNOSTICS_ENABLED)
    logger.info("[v4.0] GEMINI_FRAME=%s, TRACK_E=%s, FIRST_MENTION_WINDOW=%.0fs",
                cfg.GEMINI_FRAME_ENABLED, cfg.TRACK_E_ENABLED, cfg.FIRST_MENTION_WINDOW_S)
    ...
    for video_path in args.video:
        if not os.path.exists(video_path):
            ...
            continue
        try:
            output_dir = run_pipeline(video_path, cfg, stage_only=args.stage)

            # [v4.0] Sensitivity analysis (post-pipeline)
            if args.sensitivity_analysis and output_dir:
                try:
                    logger.info("--- Sensitivity Analysis ---")
                    from .utils.sensitivity_analysis import run_sensitivity_analysis
                    run_sensitivity_analysis(output_dir)
                except Exception as e:
                    logger.warning("Sensitivity analysis failed: %s", e, exc_info=True)
